...
│   ├── codex-reviewer.sh        # OpenAI Codex reviewer backend
...
│   ├── git-ops.sh               # Git branch, commit, push, and worktree operations
...
│   ├── worktree-cleanup.sh      # Worktree cleanup after merge and retry exhaustion
...
# Worktrees
AUTOPILOT_USE_WORKTREES="true"                     # Use git worktrees for task isolation
AUTOPILOT_WORKTREE_SETUP_CMD=""                    # Custom post-worktree-creation command
AUTOPILOT_WORKTREE_SETUP_OPTIONAL="false"          # If true, failing setup is non-fatal

# Codex reviewer (optional)
AUTOPILOT_CODEX_MODEL="o4-mini"                    # Codex model for review
AUTOPILOT_CODEX_MIN_CONFIDENCE="0.7"               # Min confidence for posting findings
