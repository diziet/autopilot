...
| `AUTOPILOT_TEST_TIMEOUT` | `300` | Timeout in seconds for test command execution |
| `AUTOPILOT_TEST_OUTPUT_TAIL` | `80` | Lines of test output included in PR comments |

Test execution timeout is controlled by `AUTOPILOT_TIMEOUT_TEST_GATE` (in the Timeouts section above). `AUTOPILOT_TEST_TIMEOUT` controls the timeout for the test command itself when run inside hooks.
