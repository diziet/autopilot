...
| `AUTOPILOT_CLAUDE_MODEL` | `claude-opus-5-5` | Claude model to use (passed via `--model` flag) |
...
| Variable | Default | Description |
|----------|---------|-------------|
| `AUTOPILOT_MAX_NETWORK_RETRIES` | <!-- fact:max-network-retries -->100<!-- /fact --> | Max retries for transient network errors before failing |
