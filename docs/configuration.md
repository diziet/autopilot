...
| `AUTOPILOT_CLAUDE_EFFORT` | `""` (empty) | Reasoning effort passed via `--effort` flag. One of `low`, `medium`, `high`, `xhigh`, `max`. Empty = don't pass the flag; the account's `settings.json` `effortLevel` applies |
